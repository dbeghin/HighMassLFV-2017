...
void IIHEAnalysis::Loop(string controlregion, string type_of_data, string out_name, string mc_nickname, Float_t nEvents) {
   ...
   vector<TH2D*> hh[h_names.size()][Mth.size()][dms.size()];
   ...
   for (unsigned int i = 0; i<h_names.size(); ++i) {
	 ...
	 for (unsigned int l = 0; l<eta.size(); ++l) {
	   TString nname = h_names[i]+"_"+Mth[j]+"_"+dms[k]+"_"+eta[l]+"_"+taun[0];
   ...
   for (Long64_t jEntry = 0; jEntry < nEntries; ++jEntry) {
	...
	for (unsigned int iMC = 0; iMC < mc_tau_had_pt->size(); ++iMC) {
      ...
      if (trig_HLT_Ele35_WPTight_Gsf_accept || trig_HLT_Ele115_CaloIdVT_GsfTrkIdT_accept)  PassTrigger = true;
	...
	if (jet_CSVv2->at(iJet) > bjetMedium2017 && jet_pt->at(iJet) > 30 && fabs(jet_eta->at(iJet)) < 2.4 && jet_isJetID_2017->at(iJet)) {
	...
	float nVert = -1, mcWeight = 1, heep_sf = 1;
	if (!data) {
	  ...
	  if (GetEtaString(ele_p4.Eta()) == "barrel") heep_sf = 0.967;
	  else if (GetEtaString(ele_p4.Eta()) == "endcap") heep_sf = 0.973;
	}
	cout << mcWeight << endl;
	...
	weightsSys = GetWeightSys(CR_number, nVert, tau_p4, ratio, ele_p4, lepton, top_pt_1, top_pt_2, mc_nick);
	first_weight = weightsSys["nominal"]*mcWeight*heep_sf;
	...
	if (CR_number == 100) {
	  if (k_dm != -1 && lMth <= k_low_SS) {
	    hh[iRatio][lMth][k_dm][l_eta]->Fill(tau_p4.Pt(), ratio, final_weight);
	  }
	  if (tau_byTightIsolationMVArun2017v2DBoldDMwLT2017->at(iTau) < 0.5) continue;
	}
   ...
   for (unsigned int i = 0; i<h_names.size(); ++i) for (unsigned int j = 0; j<k_low_SS+1; ++j) for (unsigned int k = 0; k<dms.size(); ++k) for (unsigned int l = 0; l<eta.size(); ++l) hh[i][j][k][l]->Write();
