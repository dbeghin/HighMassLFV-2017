...
void IIHEAnalysis::Loop(string controlregion, string type_of_data, string out_name, string mc_nickname, Float_t nEvents) {
   ...
   for (Long64_t jEntry = 0; jEntry < nEntries; ++jEntry) {
	...
	bool found_1 = false, found_2 = false;
        ...
        if (l1_pdgid == -l2_pdgid && found_1 && found_2) {
          ll_p4 = l1_p4 + l2_p4;
          if (ll_p4.M() > 100) reject_event = true; //for DY
        }
        else {
	...
	if (gsf_VID_heepElectronID_HEEPV70->at(iEle) && gsf_pt->at(iEle) > 35) electron = true;
	if (electron) break;
      }
	...
	if (mu_isHighPtMuon->at(iMu) && mu_ibt_pt->at(iMu) > 10 && fabs(mu_ibt_eta->at(iMu)) < 2.4 && mu_isoTrackerBased03->at(iMu) < 0.15) {
          mu1_p4.SetPtEtaPhiM(mu_ibt_pt->at(iMu), mu_ibt_eta->at(iMu), mu_ibt_phi->at(iMu), mu_mass);
          for (unsigned int iMu2= 0; iMu2 < iMu; ++iMu2) {
	    if (mu_isHighPtMuon->at(iMu2) && mu_ibt_pt->at(iMu2) > 10 && fabs(mu_ibt_eta->at(iMu2)) < 2.4 && mu_isoTrackerBased03->at(iMu2) < 0.15) {
	  ...
	  if (jet_DeepCSV->at(iJet) > bjetMedium2017 && jet_pt->at(iJet) > 30 && fabs(jet_eta->at(iJet)) < 2.4 && jet_isJetID_2017->at(iJet)) {
	...
	for (unsigned int k_syst=0; k_syst<systs.size(); ++k_syst) {
	  ...
	  cout << "weightPDF= " << weightPDF << ", " << iMedian << endl; 
	  ...
	  cout << "tau: " << lepton << "  pt: " << tau_p4.Pt() << "  ratio: " << ratio << endl;
	  cout << "mu: " << mu_lepton << "  pt: " << mu_p4.Pt() << endl << endl;
   ...
   for (unsigned int k = 0; k<systs.size(); ++k) {
     ...
     if (CR_number == 100 && !signal) {
       for (unsigned int i = 0; i<h_names.size(); ++i) for (unsigned int j = 0; j<k_low_SS+1; ++j) for (unsigned int l = 0; l<eta.size(); ++l) hh[i][j][k][l]->Write();
     }
     d_sys[k]->Close();
   }
   file_out->Close();
