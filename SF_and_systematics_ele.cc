...
map<TString, float> GetWeightSys(int CR_number, int n_vert, TLorentzVector tau_p4, float ratio, TLorentzVector ele_p4, TString lepton, float top_pt_1, float top_pt_2, TString mc_nick) {
  ...
  for (unsigned int i = 0; i<systematics.size(); ++i) {
    ...
    if (bSetto1) {
      ...
      simple_weights[key_nom] = GeneralWeightFunction(systematics[i],n_vert,tau_p4,ratio,ele_p4,lepton,top_pt_1,top_pt_2, mc_nick,"nom");
    }
	...
	simple_weights[key] = GeneralWeightFunction(systematics[i],n_vert,tau_p4,ratio,ele_p4,lepton,top_pt_1,top_pt_2, mc_nick, variations[j]);
      }
    }
